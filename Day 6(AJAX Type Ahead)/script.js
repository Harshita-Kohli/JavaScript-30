...
function findMatches(cities, wordToMatch)
{
    return cities.filter(place=>{
        const regex = RegExp(wordToMatch,'gi'); //we need to generate a regular expression from wordToMatch because the 
        // wrord to match will not always be 'newyork' or 'boston' etc...it can be anyting. so we to generate a 
        // general regex. 
...
====================//convert population to no with commas=====================
    ...
    const html = matcharray.map(place=>
        {
            ...
            `;
        }).join('');//map() returns an array with multiple items but we want a single string in suggestions, so use join()
